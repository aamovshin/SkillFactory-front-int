...
// Tabs
const BENEFITS_NAVS = document.querySelectorAll('.features__nav-item');
const BENEFITS_TABS = document.querySelectorAll('.features__item');

BENEFITS_NAVS.forEach((nav, index) => nav.addEventListener('click', event => {
  BENEFITS_NAVS.forEach(el => {
    el.classList.remove('active');
  });

  BENEFITS_TABS.forEach(el => {
    el.classList.remove('active');
  });

  nav.classList.add('active');
  BENEFITS_TABS[index].classList.add('active');
}));
